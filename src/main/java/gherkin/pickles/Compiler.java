...
public class Compiler {
    ...
    public List<Pickle> compile(GherkinDocument gherkinDocument, String path) {
        ...
        for (ScenarioDefinition scenarioDefinition : feature.getChildren()) {
            if (scenarioDefinition instanceof Background) {
                ...
                compileScenario(pickles,
                    backgroundSteps,
                    (Scenario) scenarioDefinition,
                    featureTags, path);
            } else {
                compileScenarioOutline(pickles,
                    backgroundSteps,
    ...
    private void compileScenario(List<Pickle> pickles,
                                 List<PickleStep> backgroundSteps,
                                 Scenario scenario,
                                 List<Tag> featureTags, String path) {
        ...
        Pickle pickle = new Pickle(
            ...
            java.util.Collections.singletonList(pickleLocation(scenario.getLocation(), path))
        );
        pickles.add(pickle);
    }

    private void compileScenarioOutline(List<Pickle> pickles,
                                        List<PickleStep> backgroundSteps,
        ...
        for (final Examples examples : scenarioOutline.getExamples()) {
            ...
            for (final TableRow values : examples.getTableBody()) {
                ...
                for (Step scenarioOutlineStep : scenarioOutline.getSteps()) {
                    String stepText = interpolate(scenarioOutlineStep.getText(),
                    ...
                    PickleStep pickleStep = new PickleStep(
                        stepText,
                        createPickleArguments(scenarioOutlineStep.getArgument(),
                            variableCells, valueCells, path),
    ...
    private List<Argument> createPickleArguments(Node argument, String path) {
        List<TableCell> noCells = java.util.Collections.emptyList();
    ...
    private List<Argument> createPickleArguments(Node argument,
                                                 ...
                                                 List<TableCell> valueCells, String path) {
    ...
    private PickleStep pickleStep(Step step, String path) {
        return new PickleStep(
            ...
            java.util.Collections.singletonList(pickleStepLocation(step, path))
        );
    }
    ...
    private PickleLocation pickleStepLocation(Step step, String path) {
        return new PickleLocation(
            ...
            step.getLocation().getColumn()
                + (step.getKeyword() != null
                ? SymbolCounter.countSymbols(step.getKeyword()) : 0)
        );
    }
